  ...
  const { path, query } = shapeKeys
    ...
    .reduce(
      ({ path, query }, { name, paramType, schema, required }) =>
        // @ts-expect-error fix later
        paramType === 'path'
          ? { path: { ...path, [name]: required ? schema : (schema as z.ZodType).optional() }, query }
          : { path, query: { ...query, [name]: required ? schema : (schema as z.ZodType).optional() } },
    ...
    content: {
      'application/json': {
        schema: instanceofZodTypeKind(schema, 'void')
          ? {}
          : instanceofZodTypeKind(schema, 'never') ||
            instanceofZodTypeKind(schema, 'undefined')
            ? { not: {} }
            : schema,
      },
    },
